...
import { retrieve, reset } from '../../actions/device/show';
import { del, loading, error } from '../../actions/device/delete';
import { list } from '../../actions/device/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.deviceCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.deviceUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
          ...
          <ScrollView>
            ...
            <Card title="Device">
              <List title="title">
                ...
                {Show.renderRow('macAddress', item['macAddress'])}
                {Show.renderRow('coordinator', item['coordinator'])}
              </List>
            </Card>
            }
...
const mapStateToProps = (state) => {
  return {
    error: state.device.show.error,
    loading: state.device.show.loading,
    retrieved: state.device.show.retrieved,
    deleteError: state.device.del.error,
    deleteLoading: state.device.del.loading,
    deleted: state.device.del.deleted,
  };
};
