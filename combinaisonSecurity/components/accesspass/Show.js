...
import { retrieve, reset } from '../../actions/accesspass/show';
import { del, loading, error } from '../../actions/accesspass/delete';
import { list } from '../../actions/accesspass/list';
    ...
    return (
        <View style={styles.actionStyle}>
          <SocialIcon
              ...
              type='plus-circle'
              iconColor='#3faab4'
              onPress={() => Actions.accesspassCreate()}
          />
          <SocialIcon
              iconSize={34}
              type='edit'
              iconColor='#3faab4'
              onPress={() => Actions.accesspassUpdate({id})}
          />
          <SocialIcon
              iconSize={34}
              type='minus-circle'
  ...
  render() {
    ...
    return (
        <View style={ {flex: 1} }>
          <ScrollView>
            {item &&
            <Card title="AccessPass">
              <List title="title">
                {Show.renderRow('id', item['@id'])}
                {Show.renderRow('pass', item['pass'])}
              </List>
            </Card>
            }
